    ...
    where T : ViewModelBase
{
    ...
    protected bool RunDropAction(DataGrid dg, DragEventArgs e, bool bExecute, T sourceItem, T targetItem,
        ...
        switch (e.DragEffects)
        {
            case DragDropEffects.Copy:
            {
                if (bExecute)
                {
                    ...
                    Console.WriteLine("Insert Item " + sourceItem);
                    Console.WriteLine("DataGrid: " + dg);
                    if (dg.DataContext is PlateViewModel plateViewModel && targetItem is PlateItemViewModel item)
                    {
                        plateViewModel.PlateItems.Add(item);
                    }
            ...
            case DragDropEffects.Move:
            {
                if (bExecute)
                {
                    ...
                    Console.WriteLine("Move Item " + sourceItem);
                    Console.WriteLine("DataGrid: " + dg);
                    if (dg.DataContext is PlateViewModel plateViewModel && targetItem is PlateItemViewModel item)
                    {
                        plateViewModel.PlateItems.Add(item);
            ...
            case DragDropEffects.Link:
            {
                if (bExecute)
                {
                    ...
                    Console.WriteLine("SwapItem " + sourceItem);
                    dg.SelectedIndex = 1;
                }
